(() => {
  ...
  const BUILD_VERSION = "2.3.5-bb1";   // 2.3.5 (2026-09-14 MLS) + 2026-09-16 badablingdiamonds widened giveaway filter
  ...
  const DEFAULTS = {
    bridgeKey:    "8336da3f-1497-4b0a-896c-2bcc97c75c00",   // badablingdiamonds
    ...
    sellerUsername: "badablingdiamonds",
    sport:        "nil",
    overlayId:    "badablingdiamonds-overlay",
  ...
  /* 2026-09-16 (badablingdiamonds build): giveaway detection widened to the
     rules already proven in the fleet (overlays/7cbreaks isGiveawayEvent,
     overlays/bigtime isGiveawayOrFreeItem) plus what bridge_events shows live:
     Whatnot giveaways arrive as "Givvy #N" at $0, which the old test (the word
     "giveaway" only) never caught. Called on the RAW title as well as the
     prefix-stripped one, because stripPrefixTitle turns "Giveaway - Diamond A"
     into "Diamond A". listing.transactionType is also checked when present. */
  const GIVEAWAY_WORDS = /\b(give ?away|givvy|givy|givey|gwp|raffle|freebie)\b/i;
  function isGiveawayLike(title, amount) {
    const s = String(title || "").trim().toLowerCase();
    const n = Number(amount || 0);
    return (
      GIVEAWAY_WORDS.test(s) ||
      s === "sale" ||
      s === "—" ||
      (/\bbookmark\b/.test(s) && n <= 0)
    );
  }
  function isGiveawayNode(n, rawTitle, title, amount) {
    const tx = String(n?.listing?.transactionType || n?.transactionType || "").toLowerCase();
    if (tx.includes("giveaway")) return true;
    return isGiveawayLike(rawTitle, amount) || isGiveawayLike(title, amount);
  }
  ...
  const TEAM_TITLE_RULES = [
    ...
    /* ---- MLS, 30 clubs. Added 2026-09-14 for the Card Cave MLS board. ------------
       Codes MUST match the overlay's own MLS table exactly.
       Alias discipline, deliberate and load-bearing:
         - NO bare city names. Every MLS city except Austin already belongs to a team
           in another league here ("nashville", "orlando", "portland", "seattle",
           "miami", "toronto", "vancouver"...), and a bare city in a card title is
           ambiguous. Full club names only, plus nicknames that exist nowhere else.
         - NO bare "united" (Atlanta, DC and Minnesota all end in it) and NO bare
           "fire" (matches "Panini Fire", "Firestorm" and similar product names).
         - "red bulls" is REQUIRED: without it "New York Red Bulls" hits the NBA alias
           "bulls" and resolves to CHI. EXPANDED_RULES sorts longest-alias-first, so
           the 9-character "red bulls" beats the 5-character "bulls".
         - Period-bearing forms are listed twice ("st. louis city" and "st louis city")
           because normalizeTitle KEEPS the period.
  ...
  (async () => {
    ...
    // NAMESPACED PER OVERLAY. whatnot.com is a single origin, so a client running two
    // Whatnot accounts would otherwise share one 'tsu.seen' map between both extensions
    // and each account's sales would look already-processed to the other one.
    ...
    while (true) {
      ...
      try {
        ...
        for (const n of nodes) {
          ...
          if (isGiveawayNode(n, rawTitle, title, amount)) {
            console.log("[TSU] skipping giveaway:", { id, rawTitle, amount });
            continue;
          }
          ...
          // repack board can match the spot from the title.
