(() => {
  ...
  //   bridge_keys.key = a615d74e-7891-4cf1-a8d9-150806b159b9, active = true, whatnot_handle = topbidcollectors
  //   overlay overlays/top-bid-collectors/index.html BRIDGE_KEY_DEFAULT = same
  //   overlay OVERLAY_ID = top-bid-collectors-overlay = DEFAULTS.overlayId below
  //   client_extension_config.key = same, seller_username = topbidcollectors, overlay_id = top-bid-collectors-overlay
  const DEFAULTS = {
  ...
  async function getConfig() {
    return new Promise((resolve) => {
      chrome.storage.sync.get(DEFAULTS, (cfg) => {
        ...
        // Split the allowlist once, here, so every consumer sees the same array.
        const sellerUsernames = sellerUsername.split(",").map(x => x.trim()).filter(Boolean);
        resolve({ bridgeKey, sport, overlayId, channel, pollMs, summaryEvery, sellerUsername, sellerUsernames });
      });
    });
  }
  ...
  (async () => {
    ...
    const allowed = (cfg.sellerUsernames && cfg.sellerUsernames.length)
      ? cfg.sellerUsernames
      : [cfg.sellerUsername];
    if (!allowed.includes(showHost)) {
      console.log(
        "[TSU] this show belongs to @" + showHost + ", which is not one of this client's " +
        "accounts (" + allowed.map(a => "@" + a).join(", ") + "). Not capturing. " +
        "You can watch other sellers freely, nothing is sent to your overlay."
      );
      return;
    }
    console.log("[TSU] host @" + showHost + " is an allowed account. Capturing.");
