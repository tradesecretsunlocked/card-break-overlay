(() => {
  ...
  const DEFAULTS = {
    ...
    bridgeKey: "f899b51686548ed97853a3f8490d388a",

    // optional: "nfl" | "nba" | "mlb" | "nil"
    sport: "",

    // optional: useful metadata
    overlayId: "Big-Time-Breaks",       // e.g. "sms"
    ...
    while (true) {
      try {
        ...
        for (const n of nodes) {
          ...
          // ===== TSU PATCH: ignore giveaways/free/non-team promo items =====
function isGiveawayOrFreeItem(rawTitle, title, amount){
  const text = `${rawTitle || ''} ${title || ''}`.toLowerCase();

  if (
    text.includes('giveaway') ||
    text.includes('give away') ||
    text.includes('givy') ||
    text.includes('freebie') ||
    text.includes('free ') ||
    text.includes('promo') ||
    text.includes('bonus') ||
    text.includes('credit') ||
    text.includes('wheel spin') ||
    text.includes('wheelspin')
  ){
    return true;
  }
  ...
  return false;
          ...
          if (isGiveawayOrFreeItem(rawTitle, title, amount)) {
  console.log("[TSU] skipping giveaway/free item:", { rawTitle, title, amount });
  continue;
}
...
// Leave code blank when sport is blank so the overlay can infer from
// the currently selected layout. This prevents AFC East -> AFC.
if (sport && sport !== "nil") {
