(() => {
   ...
   * sport     : nfl
   * bridgeKey : ff93638f-52bc-476c-be54-22529596c578
   * ============================================================
   */
  ...
  async function sendEvent(cfg, payload) {
    ...
    try {
      ...
      if (!r.ok) console.warn("[TSU:218-cards-co] POST failed:", r.status, await r.text().catch(()=>""));
      else console.log("[TSU:218-cards-co] sent:", body.type, body);
    } catch (e) { console.warn("[TSU:218-cards-co] POST error:", e?.message || e); }
  }
  ...
  window.addEventListener("message", (event) => {
    if (event.source !== window) return;
    if (event.data?.type === "WHATNOT_SPY_INJECTED_READY") { injectedReady = true; console.log("[TSU:218-cards-co] injected.js ready"); }
  });

  (async () => {
    for (let i = 0; i < 40 && !injectedReady; i++) await sleep(250);
    if (!injectedReady) { console.warn("[TSU:218-cards-co] injected.js never ready."); return; }

    const liveId = getLiveIdFromUrl();
    if (!liveId) { console.warn("[TSU:218-cards-co] No liveId in URL."); return; }

    const cfg = await getConfig();
    if (!cfg.bridgeUrl) { console.warn("[TSU:218-cards-co] Missing bridgeUrl."); return; }

    console.log("[TSU:218-cards-co] ── Starting ──");
    console.log("[TSU:218-cards-co] bridge:", cfg.bridgeUrl);
    console.log("[TSU:218-cards-co] key:   ", cfg.bridgeKey.slice(0,8) + "…");
    console.log("[TSU:218-cards-co] sport: ", cfg.sport);
    ...
    while (true) {
      try {
        ...
        for (const n of nodes) {
          ...
          const sport = (cfg.sport && cfg.sport !== "nil") ? cfg.sport : "nfl"; // 218 is NFL-only
