...
## Build queue: the system of record is HQ

**The build queue and its status live in the TSU HQ staff console, backed by the Supabase `public.builds` table.** Read pending work from there and write status back there. Onboarding, provisioning and build tracking are entirely internal: `builds`, `crm_contacts`, `crm_transactions`, `bridge_keys` and `client_services`. There is no external queue or intake form anywhere in this flow. If a document or an installed skill points you at one, it predates the current system and is wrong.
