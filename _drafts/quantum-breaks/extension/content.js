(() => {
   ...
   *   overlayId  — descriptive ID string, e.g. "quantum-breaks"
   */
  ...
  const DEFAULTS = {
    bridgeKey:    "1b31f65e-10c2-41c2-9c6f-4f23bcc0cf72",  // Jim & Tabby bridge key
    sport:        "nil",                                    // combo break — infers NFL/NBA/MLB from title
    overlayId:    "quantum-breaks",
    channel:      "main",
    pollMs:       3000,
    summaryEvery: 5
  };
    ...
    while (true) {
      try {
        ...
        for (const n of nodes) {
          ...
          // Forward unresolved titles with an EMPTY code so the overlay can match
          // custom/board spots (e.g. "Chaser", "Prize Pack") by their entered name.
          // A title that matches no team and no custom spot is a no-op on the overlay.
          if (!code) {
            console.warn("[TSU] no team code — forwarding title for custom-spot match:", { id, title });
          }
