(() => {
  ...
  // Crunch Zone Sports Cards client config
  const DEFAULTS = {
    bridgeKey:    "c7d707420f471348def8372bd898590c",
    sport:        "nil",
  ...
  function isBadTitle(t) {
    ...
    if (/^#?\d+$/.test(raw)) return true;  // rejects Whatnot slot-index subtitles like "#3"
    ...
    while (true) {
      try {
        ...
        for (const n of nodes) {
          ...
          // ── Draft-spot support: titles like "Spot 7" / "Draft Spot 7" / "#7" ──
          let spotNo = 0;
          if (!code) {
            const sm = String(title).match(/^\s*(?:draft\s*)?spot\s*#?\s*(\d+)\b/i) || String(title).match(/^\s*#\s*(\d+)\b/);
            if (sm) spotNo = parseInt(sm[1], 10);
          }
          const finalCode = code || (spotNo ? ("SPOT_" + spotNo) : "");
          ...
          const isSlotCode  = (c) => !c || c.startsWith("CUSTOM_") || c.startsWith("SPOT_");
          const prevIsReal  = prevCode && !isSlotCode(prevCode);
          const newIsReal   = !isSlotCode(finalCode);

          if (prevIsReal && newIsReal && prevCode !== finalCode) {
          ...
          if (ok) {
            ...
            lastCodeByItem.set(id, finalCode);
            dirty = true;
