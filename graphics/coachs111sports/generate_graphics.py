...
def resolve_logo(man):
    ...
    if url:
        # Download, then save with the extension matching the ACTUAL bytes.
        # OpenAI rejects a JPEG named .png ("invalid image file"), so sniff the magic number.
        ...
        if sig[:3] == b"\xff\xd8\xff":
            ...
            ext = "png"
        dest = os.path.join(BUILD_DIR, "_logo." + ext)
        with open(dest, "wb") as out:
            out.write(data)
        print("  saved logo as %s" % os.path.basename(dest))
        return dest
    return None
...
def _norm(path):
    """OpenAI's image edits endpoint wants a clean RGBA PNG. A raw JPEG (or a CMYK /
    palette-mode image) triggers 'invalid image file or mode'. Convert every reference
    image to RGBA PNG first."""
    try:
        from PIL import Image
    except ImportError:
        die("Pillow is needed to prepare the logo/reference images.\n"
            "       Install it once, then re-run:   pip install Pillow")
    out = os.path.splitext(path)[0] + ".rgba.png"
    try:
        Image.open(path).convert("RGBA").save(out, "PNG")
        return out
    except Exception as e:  # noqa
        print("  WARN: could not normalize %s (%s); using original" % (os.path.basename(path), e))
        return path
